"""Resolve only a verified synthetic Meizu item; open the existing cache read-only."""
import hashlib
...
def browser_target(scope, root, account, platform='meizu'):
    name, index = scope.get('manifest'), scope.get('index')
    _require(platform == 'meizu')
    ...
    with closing(sqlite3.connect(database.resolve().as_uri() + '?mode=ro', uri=True, timeout=15)) as db:
        ...
        if source.source_folder_name:
            ...
            # available_name truncates the stem to 16 UTF-16 units before adding a
            # new suffix; a migrated numbered group can therefore end in "M ( (2)".
            ...
            pattern = r'未分类(?: \(\d+\))?' if original_default else r'笔记互迁分组验收 [A-Z0-9 -]+(?: \(\d+\)){0,4}(?: \(\d* \(\d+\))?'
            _require(actual.source_folder_name and re.fullmatch(pattern, actual.source_folder_name)
                     and len(actual.source_folder_name.encode('utf-16-le')) // 2 <= 16)
            if original_default:
        ...
        return {'fixtureId': identity, 'title': actual.display_title, 'expectedText': actual.plain_text,
                ...
                'group_id': actual.source_folder_id or '-1', 'headings': [
