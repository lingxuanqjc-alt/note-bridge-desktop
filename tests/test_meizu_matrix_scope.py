...
from note_bridge.providers.meizu import encode_blocks, parse_entry
from note_bridge.providers.meizu_groups import folder_key
from note_bridge.storage import Store

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / 'scripts'))
spec = importlib.util.spec_from_file_location('meizu_matrix_scope', ROOT / 'scripts/meizu_matrix_scope.py')
...
def scope(tmp_path, monkeypatch):
    ...
    seed = NoteDocument(platform=PlatformId.VIVO, account_id='synthetic', source_id='seed',
                        title='笔记互迁验收 · 魅族范围测试', source_folder_id='source-group',
    ...
    actual = parse_entry({'uuid': target_id, 'title': source.title,
                          'body': json.dumps(encode_blocks(source, {asset.id: asset.id})[0])},
                         'target', group_id, source.source_folder_name)
    ...
    target_provider = SimpleNamespace(spec=SimpleNamespace(id='meizu'), account_id='target')
    ...
    manifest = {'id': 'matrix-20260907-scope', 'kind': 'cloud-matrix-batch', 'target': 'meizu', 'expected_account': 'target', 'armed': False,
    ...
    evidence = {'batch_id': manifest['id'], 'target': 'meizu', 'status': 'api_verified', 'original_target_notes_unchanged': True,
...
def test_another_source_cannot_claim_oppo_default_group_by_name_or_forged_scope_flag(scope):
    ...
    scope.store.replace_snapshot('meizu', 'target', [scope.actual], True)
...
def test_verified_multihop_group_keeps_truncated_stem_before_numbered_suffix(scope):
    scope.actual.source_folder_name = '笔记互迁分组验收 M ( (2)'
    scope.store.replace_snapshot('meizu', 'target', [scope.actual], True)
...
def test_unrelated_or_unverified_state_is_rejected_before_browser_launch(scope, failure):
    ...
    if failure == 'account':
        ...
        scope.store.replace_snapshot('meizu', 'target', [scope.actual], False)
